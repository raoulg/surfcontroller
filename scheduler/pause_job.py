...
def run_pause_job():
    ...
    try:
        ...
        for vm in vms:
            if vm.active:
                if vm.id in exclusions:
                    logger.info(
                        f"Skipping {vm.name} ({vm.id}) - Excluded from schedule"
                    )
                else:
        ...
        if vms_to_pause:
            logger.info(
                f"Pausing {len(vms_to_pause)} VMs: {', '.join(vm_names_to_pause)}"
            )
            try:
        ...
        logger.error(f"Unexpected error in pause job: {e}")
        log_status(f"FAILED: Unexpected error ({e})")
